...
const { ZcashBlock, fromHashHex } = require('zcash-block')
...
function encodeInit (multiformats) {
  return function encode (obj) {
    ...
    return ZcashBlock.fromPorcelain(Object.assign({}, obj, { tx: null })).encode()
  }
}
...
function decodeInit (multiformats) {
  return function decode (buf) {
    ...
    const deserialized = ZcashBlock.decodeHeaderOnly(buf, true).toPorcelain()
 ...
 *
 * The process of converting to a CID involves reversing the hash (to little-endian form), encoding as a `dbl-sha2-256` multihash and encoding as a `bzcash-block` multicodec. This process is reversable, see {@link cidToHash}.
 *
 * @param {object} multiformats a multiformats object with `dbl-sha2-256` multihash and `zcash-block` multicodec registered
